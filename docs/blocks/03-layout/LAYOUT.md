...
| **(신규) 사업 임팩트** | <!-- TODO: 사용자 채움 --> | <!-- 후보: "GT-SS500 양산 출하 / 매출 K억" / "주 N시간 자동화 절약" / "0→1 풀스택 PoC X일" --> | EXPERT_REVIEW §4 권장 |
...
| 우선순위 | <!-- TODO: 사용자 결정 — P1 (즉시) / P2 (B4 후) / P3 (B5) --> |
...
| 우선순위 | <!-- TODO: 사용자 결정 — P1 (대기업 PM 트랙 지원 시) / P2 (B5에서 이력서와 동시) --> |
...
| 한의학·동양철학 이력 | About에서 노출 안 함 (B2 §5.2 후보) | <!-- TODO: 사용자 결정 --> |
...
| # | 항목 | 위치 | 영향 |
|---|------|------|------|
| 1 | Hero 4번째 수치카드 (사업 임팩트) | §2.3 | M1 정량성 |
| 2 | `/impact` 페이지 신설 여부 | §2.4 | 옵션 C 채택 |
| 3 | `cases/autonomy-stack/` 우선순위 | §3.1 | B4 컴포넌트 신규 |
| 4 | `cases/apqp-rmci/` 우선순위 | §3.2 | B4 컴포넌트 신규 |
| 5 | Nav 구조 (N1/N2/N3) | §4.4 | 전체 페이지 헤더 |
| 6 | Hero 헤드라인 (H-A/H-B/H-C) | §5.1 | M1 메시지 |
| 7 | About 기술 스택 보강 항목 (6 분류) | §5.2 | 도메인 어휘 매칭 |
| 8 | 한의학·동양철학 이력 컷 여부 | §5.4 | B2 §5.2 |
...
- [ ] §1.1 5섹션 와이어프레임 표 확정 (섹션 ID + 컴포넌트 매핑)
- [ ] §1.3 결정표 (현 4 카드 vs V3 5 테마) 확정
- [ ] §2.2 Impact Dashboard 위치 결정 (A+C 채택 또는 변경)
- [ ] §2.3 Hero 4번째 수치카드 사용자 채움
- [ ] §3.1 / §3.2 신규 케이스 페이지 P1/P2/P3 결정
- [ ] §4.3 케이스 하단 "다음" 정책 확정 (L1/L2/L3)
- [ ] §4.4 상단 Nav 안 확정 (N1/N2/N3)
- [ ] §5.1 Hero 헤드라인 1개 확정 (타겟 회사 매칭)
- [ ] §5.2 About 기술 스택 보강 항목 사용자 선택
- [ ] §5.4 한의학·동양철학 이력 컷 결정
- [ ] §6.1 B4로 넘기는 결정 10항목 표기
- [ ] §6.3 사용자 직접 결정 8항목 채움
