...
| # | 태스크 | 우선순위 | depends | 비고 |
|---|--------|:--------:|---------|------|
| B3-01 | 5섹션 와이어프레임 확정 (THEME_MAP_V3 기반) | P1 | 사용자 입력 | `LAYOUT.md` 골격 완료. Hero 후보 3 / Impact 위치 / 신규 케이스 후보 사용자 결정 대기 |
| B3-02 | Impact Dashboard 위치 결정 + 페이지 간 내비게이션 | P2 | B3-01 ✅ | A+C 병행·L3·N2 확정. B4 진입 전 Hero 헤드라인(§5.1)만 B2 후 결정 |

## B4 — 디자인 (1개)

| # | 태스크 | 우선순위 | depends | 비고 |
|---|--------|:--------:|---------|------|
| B4-01 | 디자인 시스템 수립 + 컴포넌트 통일 + astro 페이지 구현 | P1 | B3-01 결정 후 | `DESIGN_SYSTEM.md` 골격 완료(10절). 5건 사용자 결정 대기 (다크모드/한글폰트/코드폰트/본문크기/Hero zoom) |
